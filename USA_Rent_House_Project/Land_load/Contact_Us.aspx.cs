    ...
    public partial class Contact_Us : System.Web.UI.Page
    {
        ...
        protected void MessageButton_Click(object sender, EventArgs e)
        {
            if (Page.IsValid == true)
            {
                ...
                if (rtn2 == 1)
                {
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "Redirect", "window.onload = function(){ alert('" + Messages.Sending_Email_success + "'); window.location = '/Land_load/Land_load_Profile.aspx'; }", true);
                }
                else
                {
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "Redirect", "window.onload = function(){ alert('" + Messages.Sending_Email_Error + "'); window.location = '/Contact-Us.aspx'; }", true);
                }
            }
        }
